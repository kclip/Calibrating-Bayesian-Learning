...
def compute_subbatches(net, images, network_type, svd=None, feature_projection=None):
    ...
    for split in splits:
        ...
        if ('FNN' in network_type):
            ...
            split_logits = net(images[split].to(device))
...
def convert_image_dataset(
    ...
    net = torch.load('C:/Users/hjkl1/PycharmProjects/selective-classification/bin/cifar/models_saved/fnn-gamma-0.0.pt')
    net = net.to(device)
    ...
    with torch.no_grad():
        for images, targets in tqdm.tqdm(loader, desc='processing dataset'):
            features, logits = compute_subbatches(net.module, images, network_type, svd)
...
def process_image_splits(args, net, splits, output_dir, network_type):
    ...
    test_dataset = datasets.CIFAR100(args.data_dir, train=False, download=True, transform=transform)
    ...
    with torch.no_grad():
        ...
        for images, labels in test_loader:
            ...
            accuracy = 100 * torch.sum(pred_label.eq(labels)) / 10000
        print(accuracy)
    ...
    #     net, loader, network_type, temperature,  keep_batches=True, svd=None)
    # a = 'train_' + network_type
    # torch.save(ds, os.path.join(output_dir, f'{a}.pt'))
    # del ds
    ...
    for split in ['cal']:
        ...
        sadasd
        ds = convert_image_dataset(
            net, loader, network_type, temperature, keep_batches=False, svd=svd)
        a = split + '_' + network_type
        torch.save(ds, os.path.join(output_dir, f'{a}.pt'))
