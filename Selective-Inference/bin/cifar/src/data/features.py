...
def get_outlier_models(train_input_features):
    ...
    np.random.seed(1)
    print(len(train_input_features))
    indices = np.random.permutation(len(train_input_features))
    print(indices)
    print(indices[:MAX_TRAIN_EXAMPLES])
    ...
    n_samples, x, y = train_input_features.shape
    print(train_input_features.shape)
    train_input_features = train_input_features.reshape(-1, 128)
    print(train_input_features.shape)
    ...
    osvm.fit(train_input_features[:100000])
    ...
    lof.fit(train_input_features[:100000])
...
def process_example(indices, skip_class_based_features=True):
...
def generate_dataset_features(
    ...
    # batches = np.arange(num_examples)
...
def process_dataset_splits(
 ...
 #  for selective calibration
    ...
    cal_meta_dataset = BatchedInputDataset(
        ...
        true_confidences=cal_dataset.true_confidences,
        labels=cal_dataset.labels,
        logits=cal_dataset.logits,
        pred_labels=cal_dataset.pred_labels,
        targets=cal_dataset.targets)
...
#         labels=cal_dataset.labels,
#         logits=cal_dataset.logits,
#         pred_lables=cal_dataset.pred_lables,
#         targets=cal_dataset.targets)

    print(cal_meta_dataset.labels)
    print(cal_meta_dataset.labels.shape)
    ...
    test_meta_datasets.append(InputDataset(
        ...
        output_probs=test_datasets.output_probs,
        confidences=test_datasets.confidences,
        true_confidences=test_datasets.true_confidences,
        labels=test_datasets.labels,
        logits=test_datasets.logits,
        pred_labels=test_datasets.pred_labels,
        targets=test_datasets.targets))


    print(f'\nDeriving OOD test features for {len(test_datasets)} datasets...')
    ood_test_meta_datasets = []
    ...
    features = generate_dataset_features(
        ...
        num_workers=num_workers)
    features = torch.from_numpy(meta_scaler.scale(features.numpy()))
    ood_test_meta_datasets.append(InputDataset(
        input_features=features,
        output_probs=ood_dataset.output_probs,
        confidences=ood_dataset.confidences,
        true_confidences=ood_dataset.true_confidences,
        labels=ood_dataset.labels,
        logits=ood_dataset.logits,
        pred_labels=ood_dataset.pred_labels,
        targets=ood_dataset.targets))
#
    ...
    #     labels=test_datasets.labels,
    #     logits=test_datasets.logits,
    #     pred_lables=test_datasets.pred_lables,
    #     targets=test_datasets.targets))
    ...
    return cal_meta_dataset, test_meta_datasets, ood_test_meta_datasets
