...
with torch.no_grad():
    ...
    for images, labels in test_loader:
        ...
        avg_prob = 0
        for _ in range(ensemble_test):
            logps = model(images)
            ps = torch.nn.functional.softmax(logps.data, dim=1)
            avg_prob += ps / ensemble_test

        confidence_id, pred_label = torch.max(avg_prob.data, dim=1)
        ...
        pf.reliability_diagram_plot(confidence_id, pred_label, labels, ECE, accuracy, network_type='mmce')
...
with torch.no_grad():
    ...
    for images, labels in uncertainty_test_loader:
        ...
        confidence_ood, pred_label = torch.max(avg_prob.data, dim=1)
...
pf.confidence_distribution_plot(confidence_id, confidence_ood, 'mmce', num_bins=15)